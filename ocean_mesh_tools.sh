...
cmd_apply() {
    ...
    if [ -z "$1" ]; then
        echo -e "${YELLOW}Uso: ./ocean_mesh_tools.sh apply <grid_file> <mask_file> [opções]${NC}"
        echo ""
        echo "Argumentos:"
        ...
        echo "  --preserve-boundaries  - Preserva colunas em -180°/+180° (recomendado para grades globais)"
        echo ""
        echo "Exemplos:"
        ...
        echo ""
        echo "  # Grade global com preservação de bordas longitudinais"
        echo "  ./ocean_mesh_tools.sh apply grid_global.asc mask.asc --preserve-boundaries"
        echo ""
